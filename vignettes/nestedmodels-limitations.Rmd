...
Yet even within its one most obvious use case, nested models have further limitations.
...
The second problem is more related to physical performance. Even when fitting an uncomplicated model to a fairly small dataset, the fitting process takes a few seconds to complete.
...
However, for some models, most notably forecasting algorithms, nestedmodels can seem like the only solution for forecasting panel data. In this specific case, a global forecasting method would be recommended (e.g. '[Prophet](https://facebook.github.io/prophet/)' or a gradient boosting model), since this can deal with categorical data. More generally, this means that if a nested and normal approach do not work for some model, this often means that the best route of action is to abandon said model, and find one that will suit your needs more effectively.
