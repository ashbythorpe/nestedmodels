...
We can then fit this model in the usual way. Note that the data must be nested.

```{r fit_nested_model}
model_fit <- fit(nested_model, z ~ ., nested_data)
```
...
We first define the recipe, and we define a step which is used to nest the data.
