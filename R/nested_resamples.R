...
#' 
#' nested_resamples(nested_data, rsample::nested_cv(rsample::vfold_cv(), 
#'                  rsample::bootstraps()))
#' 
#' @export
nested_resamples <- function(data, resamples, nesting_method = NULL, 
                             size_action = c("truncate", "recycle",
  ...
  if(!is.null(nesting_method) && 
    ...
    stop_bad_type("nesting_method", "a recipe, workflow or NULL", 
                  nesting_method)
  }
...
create_resamples <- function(x, data, resamples, action, env, ...) {
  ...
  if(inherits(actual_resamples[[1]], "nested_cv")) {
    ...
    purrr::map(res$rset, "splits") %>%
      purrr::pmap( ~ {combine_rsets(list(...), x = x, data = data, 
