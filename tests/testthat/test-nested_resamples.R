test_that("nested_resamples works", {
  ...
  nested_resamples(dplyr::group_by(example_nested_data, id), 
                   ~ rsample::initial_split(.)) %>%
    expect_s3_class(class(rsample::initial_split(example_nested_data)))
  nested_resamples(example_nested_data, ~ {rsample::validation_split(.)}, 
                   nesting_method = recipe) %>%
    expect_s3_class(class(rsample::validation_split(example_nested_data)))
  nested_resamples(example_nested_data, rsample::bootstraps, times = 25, 
                   nesting_method = wf) %>%
  ...
  nested_resamples(tidyr::nest(small_data, data = -.data$id), 
                   rsample::nested_cv(rsample::vfold_cv(), 
                                      rsample::bootstraps())) %>%
    expect_s3_class(class(rsample::nested_cv(small_data,
                                             rsample::vfold_cv(),
                                             rsample::bootstraps())))
})
